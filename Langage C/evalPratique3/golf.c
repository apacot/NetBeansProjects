...
/*
  ...
  *		ficherInfoTrou( typeTrou unTrou);
  *  en entrées :
  *		
  *  paramètre de sortie
  *		
...
int jouerTrou( typeTrou t, float difficulte )
{
	...
	switch ( nbCoups - t.par )
	{
		...
		case CONDOR:
			printf( "%s CONDOR !!!! %d\n%s", YELLOW, CONDOR, BLACK );
			break;

		case ALABATROS:
			printf( "%s ALBATROS!!! %d\n%s", YELLOW, ALABATROS, BLACK );
			break;
		...
		case DOUBLE_BOGEY:
			printf( "%sDOUBLE_BOGEY +%d%s\n", RED, DOUBLE_BOGEY, BLACK );
			break;

		case TRIPLE_BOGEY:
			printf( "%sTRIPLE_BOGEY +%d%s\n", RED, TRIPLE_BOGEY, BLACK );
			break;

		default:
			printf( "%s+%d%s\n", RED, nbCoups - t.par, BLACK );
	}
...
{
    printf("Le numéro du trou actuel est : %d\n", unTrou.numero);
    printf("La distance qui vous sépare du trou est de : %.1f metres\n", unTrou.distance);
    printf("La valeur du par pour ce trou est : %d\n", unTrou.par);
    printf("Le nombre de coups maximum pour ce trou est : %d\n", unTrou.nbMaxCoups);
...
void afficherLesParcours( typeParcours *tab[], int nbParcours )
{
    ...
    printf("Il y a %d parcours disponibles\n",nbParcours);
    for (i=0;i<nbParcours;i++)
    {
        printf("nom du parcours numéro %d : %s\n",i,tab[i]->nom);
    }
...
void sauvegarderParcours( typeParcours *tab[], int nbParcours )
{
    ...
    ficSave = fopen(FICHIER_PARCOURS,"w+");
    if (ficSave == NULL) 
    {
        printf("%s\n", strerror(errno));
        exit(errno);
    ...
    //sauvegarde de chacun des parcours
    for (i=0; i<nbParcours; i++)
    {
        retour = fwrite(tab[i],sizeof (typeParcours),1,ficSave);
        if (retour != 1)
        {
        printf("%s\n", strerror(errno));
        exit(errno);
...
void jouerParcoursADeux( typeParcours *p )
{
    ...
    scJ2->difficulte = scJ1->difficulte;
...
{       
    //si nombre de parcours > 0, affiche le menu complet (jouer / creer / quitter)
        if (nbParcours> 0)
        {
        ...
        } //sinon affiche seulement le menu pour creer un parcours ou quitter
